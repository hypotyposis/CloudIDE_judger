...
def compile():
    ...
    print(code_value, cin_value, timestamp, language)
    # f = open("1.in", "w")
    ...
    f = open(timestamp + language_postfix[language], "w")
    f.write(code_value)
    f.close()
    if language == "python":
        p = subprocess.Popen(args="timeout -s 9 2s python3"+" "+timestamp+".py" + "<" + timestamp + ".in", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    #编译C++代码
    p = subprocess.Popen(args="g++" + " " + timestamp + language_postfix[language] + " -o " + timestamp, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    out_put_logs = list()
    while p.poll() is None:
        out_put_log = str(p.stdout.readline(), encoding="utf-8")
        out_put_logs.append(out_put_log)
